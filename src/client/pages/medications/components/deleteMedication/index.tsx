...
function DeleteMedication() {
  const { modalIsOpen, modalData, closeModal } = useModal("delete-medication");
  ...
  return (
    <FormProvider form={<Form method="post" />}>
      <ModalContainer isVisible={modalIsOpen} makeInvisible={closeModal}>
        <ModalHeader>Remover medicamento</ModalHeader>

        <ModalContent>
          <input type="hidden" name="id" value={modalData?.id} />
          <DashedBorderCircle icon={<Trash2 />} />
          <div>
            <strong>Deseja remover este medicamento?</strong>
            <p>
              O medicamento <b>{modalData?.name}</b> será removido
              permanentemente.
            </p>
          </div>
        </ModalContent>
